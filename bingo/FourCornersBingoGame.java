...
public class FourCornersBingoGame implements Winnable {
	...
	public FourCornersBingoGame(BingoCard user, BingoCard computer) {
	...
	public boolean gameOver() {
		...
		if (this.isUserWin && userWins != null && !this.userWins.equals("") && this.isComputerWin
				&& computerWins != null && !this.computerWins.equals("")) {
	...
	private boolean validateIfComputerWins(BingoCard bingoCard, int check) {
		...
		for (int i = 0; i < 5; i++) {
			for (int j = 0; j < 5; j++) {
				if (i == 0 && j == 0 && marked[i][j] == true) {
					count++;
				}

				if (i == 0 && j == 4 && marked[i][j] == true) {
					count++;
				}

				if (i == 4 && j == 0 && marked[i][j] == true) {
					count++;
				}

				if (i == 4 && j == 4 && marked[i][j] == true) {
					count++;
				}
				if (count == 4) {
					isComputerwin = true;
